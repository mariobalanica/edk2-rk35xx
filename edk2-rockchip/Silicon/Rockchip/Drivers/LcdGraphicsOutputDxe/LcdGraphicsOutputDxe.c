  ...
  )
{
  ...
  DrmDisplayMode->Clock      = DisplayMode->OscFreq / 1000;
  ...
  )
{
  ...
  for (Index = 0; Index < ConnectorCount; Index++) {
    ...
    DisplayState->ConnectorState.Connector = (VOID *)Connector;

    if ((ModePreset->Preset == DISPLAY_MODE_NATIVE) && (NativeMode == NULL)) {
      //
      // Get predefined mode from panel.
      //
      Status = gBS->HandleProtocol (
                      ConnectorHandles[Index],
                      &gRockchipPanelDisplayModeProtocolGuid,
                      (VOID **)&NativeMode
                      );
    ...
    DisplayState->CrtcState.CrtcID = 2;
    ...
    Crtc->Vps[DisplayState->CrtcState.CrtcID].Enable = TRUE;
  ...
  if (ModePreset->Preset == DISPLAY_MODE_NATIVE) {
    ...
    if (Mode != NULL) {
          ...
          (Mode->OscFreq > VOP_PIXEL_CLOCK_MAX))
      {
        ...
        DEBUG ((
          DEBUG_ERROR,
          "%a: Custom mode unsupported! (OscFreq=%u)\n",
          __func__,
          Mode->OscFreq
          ));
        ASSERT (FALSE);
