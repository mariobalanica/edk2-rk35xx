  ...
  )
{
  ...
  /* todo: override quantization range if supported? */
  ...
  )
{
   ...
   * VIC > 1 expect limited range. Ideally we'd set VIC to 0,
  ...
  if (!SinkInfo->IsHdmi20 && (CeaVic > 64)) {
    CeaVic = 0;
  }
  ...
  )
{
  ...
  if (SinkInfo->IsHdmi) {
    if (!Hdmi->ForceDviMode) {
      DEBUG ((DEBUG_INFO, "%a: Using HDMI mode\n", __func__));
      DwHdmiQpRegMod (Hdmi, 0, OPMODE_DVI, LINK_CONFIG0);
    } else {
      DEBUG ((DEBUG_INFO, "%a: Forcing DVI mode for HDMI sink\n", __func__));
      DwHdmiQpRegMod (Hdmi, OPMODE_DVI, OPMODE_DVI, LINK_CONFIG0);
    }
    ...
    if (ConnectorState->DisplayMode.Clock > 340000) {
       ...
       * Under normal circumstances (following EDID) we don't expect to see
       * an unsupported mode.
       */
    ...
    DEBUG ((DEBUG_INFO, "%a: Using DVI mode\n", __func__));
  ...
  )
{
  ...
  for (Index = 0; Index < ARRAY_SIZE (mRk3588DwHdmiQpDevices); Index++) {
    ...
    Hdmi->ForceDviMode = PcdGetBool (PcdHdmiForceDviMode);
