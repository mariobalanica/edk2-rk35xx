  ...
  )
{
  ...
  if (SinkInfo->IsHdmi20) {
    InfBuf[8] = 0x1 << 6; /* YCC Quantization Range = Full */
  }
  ...
  )
{
   ...
   */
  if ((HdmiVic > 0) || (!SinkInfo->IsHdmi20 && (CeaVic > 64))) {
    CeaVic = 0;
  }
  ...
  )
{
  ...
  if (HdmiMode == SinkInfo->IsHdmi) {
    ...
    DEBUG ((
      DEBUG_INFO,
      "%a: Forcing %a mode for %a sink\n",
      __func__,
      HdmiMode ? "HDMI" : "DVI",
      SinkInfo->IsHdmi ? "HDMI" : "DVI"
      ));
  }
